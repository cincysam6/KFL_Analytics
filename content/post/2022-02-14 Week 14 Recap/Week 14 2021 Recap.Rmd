...
wk4_matchups%>%mutate(win_prob=round(win_prob,2),opp_win_prob=round(opp_win_prob,2))%>%gt()%>%gt_img_rows(logo)%>%gt_img_rows(opponent_logo)%>%
  tab_header(title ="Week 14 matchups")%>%
...
## Expected Wins power rankings through week 14
...
)+coord_flip()+ggtitle("Expected Wins through Week 14") + xlab("franchise") + ylab("expected wins")+theme(axis.title.x = element_text(size=14),axis.title.y=element_text(size=14),axis.text = element_text(size = 12),plot.title = element_text(size = 18))

week4_exp_wins

################### UPDATE TABLE TITLE ##############################
week_4_table%>%select(franchise_name,logo,expected_wins,wins_over_expected)%>%gt()%>%gt_img_rows(logo) %>%tab_header(title ="Expected Wins through Week 14")%>%
...
standings_df<-joined_df%>%select(franchise_name,logo,division_name,Wins,Losses,outcomes)%>%arrange(desc(Wins))%>%gt(groupname_col="division_name")%>%gt_plt_winloss(outcomes,max_wins = 14)%>%gt_img_rows(logo)%>%tab_header(title ="Standings Through Week 14")%>%cols_label(franchise_name="Team",logo="",outcomes="Win-Loss Streak")
