---
title: "Week 6 2022 Recap"
author: "Samuel Kirschner"
date: "2022-10-17T13:09:13-06:00"
...
This week was a low scoring affair with zero games above 100 points. The "UN"Lucky ducks finally getting knocked off by the newly named Death Monarchs restoring a bit of parity to the league. LGTE continue their slide with a loss to the Kraken who now share the division lead with Nick. The Tenderloin's continue to lose as they fell to the Titan's and the GMO's found a way to win over the Sunday Winner's despite only putting up 63. Finally, the Slugs had a chance to catch up to the Bluecats, but lost to the Onionheads who take sole lead in the Glum division.
...
wk4_matchups%>%mutate(win_prob=round(win_prob,2),opp_win_prob=round(opp_win_prob,2))%>%gt()%>%gt_img_rows(logo)%>%gt_img_rows(opponent_logo)%>%
  tab_header(title ="Week 6 2022 matchups")%>%
...
## Expected Wins power rankings through week 6 2022

The Onionheads and LGTE are now tied at the top with 3.7 expected wins, while the Kraken and Death Monarchs close the gap both above 3 expected wins. The 5-1 "UN"Lucky Ducks move to 6th but are still the luckiest team with 2.4 wins above expected. At the bottom of our ranks, the Tenderloins finally broke the 1 expected win threshold!
...
A bit more parity now in the standings as we are down to just two 1-5 teams  and no undefeated teams. The Kraken and Ducks are tied at 5-1 in the Kurt division while the Onionheads take sole lead at 5-1. The Ohio division could be interesting as the Death Monarchs close the gap 2 games behind the division leading Bluecats.
...
standings_df<-joined_df%>%select(franchise_name,logo,division_name,Wins,Losses,outcomes)%>%arrange(desc(Wins))%>%gt(groupname_col="division_name")%>%gt_plt_winloss(outcomes,max_wins = 14)%>%gt_img_rows(logo)%>%tab_header(title ="Standings Through Week 6 ")%>%cols_label(franchise_name="Team",logo="",outcomes="Win-Loss Streak")

standings_df

```

### Best Ball Week 6 Update
...
Hey, the Tenderloin's need a win somewhere so I guess Teddy Bridgewater will be our stud of the week?
...
Kyler Murray is our Dud of the week..... Did the new Call of Duty just come out????
