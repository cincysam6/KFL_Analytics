---
title: "Week 8 2021 Recap"
author: "Sam Kirschner"
date: 2021-11-14T13:09:13-06:00
...
wk4_matchups%>%mutate(win_prob=round(win_prob,2),opp_win_prob=round(opp_win_prob,2))%>%gt()%>%gt_img_rows(logo)%>%gt_img_rows(opponent_logo)%>%
  tab_header(title ="Week 8 matchups")%>%
...
## Expected Wins power rankings through week 8

Week 8 sees the Ohio and Indiana division pull away with two top tier teams. Coming off there big trades the Supermarket Studs sit in our top spot with 5.3 expected wins. The Slugs and Kraken Tie for second with 5.1 and Swab rounds out the top 4 at 5. The Unlucky Ducks are at a full 3.7 wins under expected with only 1 win on the season but 4.7 expected wins. It is truly shaping up to be one of the toughest luck seasons for the Ducks. Scott's Titans on the other hand, enjoy a division lead despite a meager 3.8 expected wins on the season. 
...
)+coord_flip()+ggtitle("Expected Wins through Week 8") + xlab("franchise") + ylab("expected wins")+theme(axis.title.x = element_text(size=14),axis.title.y=element_text(size=14),axis.text = element_text(size = 12),plot.title = element_text(size = 18))

week4_exp_wins

################### UPDATE TABLE TITLE ##############################
week_4_table%>%select(franchise_name,logo,expected_wins,wins_over_expected)%>%gt()%>%gt_img_rows(logo) %>%tab_header(title ="Expected Wins through Week 8")%>%
...
Supermarket studs pull out to a 2 game lead in the Indiana Division. Meanwhile, a lot of action in the Ohio division with two 6-2 teams followed by the Bluecats clawing back t0 .500 and the poor Sunday Winners at the bottom of the division despite 4.8 expected wins on the season. The SWAB did have their 6 game win streak broken and will look to get back on track in week 9. Finally, the Glum division is lead by the Titans who at 5-3 are the only team with a winning record in the divison. That is your KFL wrap up for week 8.
...
standings_df<-joined_df%>%select(franchise_name,logo,division_name,Wins,Losses,outcomes)%>%arrange(desc(Wins))%>%gt(groupname_col="division_name")%>%gt_plt_winloss(outcomes,max_wins = 14)%>%gt_img_rows(logo)%>%tab_header(title ="Standings Through Week 8")%>%cols_label(franchise_name="Team",logo="",outcomes="Win-Loss Streak")
