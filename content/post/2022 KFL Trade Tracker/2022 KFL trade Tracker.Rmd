...
total_points_post<-as.data.frame(total_score[c(5:8),]%>%select(franchise_name.x,logo,total_points)%>%group_by(franchise_name.x,logo)%>%mutate(total_team_value = sum(total_points))%>%select(-total_points,-player_name)%>%distinct())
