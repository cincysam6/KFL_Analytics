---
title: "Week 2 2022 Recap"
author: "Samuel Kirschner"
date: "2022-09-20T13:09:13-06:00"
...
The Bluecats get another fortunate win putting up less than 80 against the Slugs who came back down to earth this week with a poor performance. The incorrectly named Unlucky Ducks got a very lucky win against an opponent who can't set their lineup and still almost lost. Meanwhile in the Glum division Elliot's Last Good Tight End put up a smash victory with almost a record setting week and the Titan's and Onionhead's had a competitive high scoring match-up making the Glum division look strong to start the season. Rounding the week out the Sunday Winner's put up an impressive performance against the Monarchs who also came back down to earth with a poor 68 point performance and the Supermarket Studs and looking like the Supermarket Duds as they lost to Kraken who were not much better. 
...
wk4_matchups%>%mutate(win_prob=round(win_prob,2),opp_win_prob=round(opp_win_prob,2))%>%gt()%>%gt_img_rows(logo)%>%gt_img_rows(opponent_logo)%>%
  tab_header(title ="Week 2 2022 matchups")%>%
...
## Expected Wins power rankings through week 2 2022

The big week from Elliot's Last Good Tight Ends (LGTE) was one of the highest we've ever seen in the KFL. This basically guaranteed a win posting an expected win value of 0.99 and catupalting them to the top of our leaderboard. Former league leader the Slugs fall to second with a big gap between them and LGTE. The Glum division looks like they may be tough this year with the Onionheads and Titans rounding out the top 4. 
We have two 2-0 teams who have benefited from getting some low scoring opponents. Nick's (Un)Lucky ducks have put up a meager 0.6 expected wins and the Bluecats aren't much better at 0.7. We will see if they can keep this luck streak alive or if they will inevitably come crashing down to earth. On the other end of the spectrum, the Monarchs should have a win by now ranking 7th with 1.01 expected wins, but sadly sit at 0-2 and will look to reverse their luck against the Bluecats next week. 
...
)+coord_flip()+ggtitle("Expected Wins through Week 2") + xlab("franchise") + ylab("expected wins")+theme(axis.title.x = element_text(size=14),axis.title.y=element_text(size=14),axis.text = element_text(size = 12),plot.title = element_text(size = 18))

week4_exp_wins

################### UPDATE TABLE TITLE ##############################
week_4_table%>%select(franchise_name,logo,expected_wins,wins_over_expected)%>%gt()%>%gt_img_rows(logo) %>%tab_header(title ="Expected Wins through Week 2")%>%
...
standings_df<-joined_df%>%select(franchise_name,logo,division_name,Wins,Losses,outcomes)%>%arrange(desc(Wins))%>%gt(groupname_col="division_name")%>%gt_plt_winloss(outcomes,max_wins = 14)%>%gt_img_rows(logo)%>%tab_header(title ="Standings Through Week 2")%>%cols_label(franchise_name="Team",logo="",outcomes="Win-Loss Streak")
...
group_by(franchise_id,franchise_name.x,logo)%>%
  mutate(total_bb_score =sum(team_score),
         ...
         bb_wins = sum(bb_win),
         bb_losses = sum(bb_loss))%>%select(-franchise_id,-bb_win,-bb_loss,-franchise_score,-team_score)%>%distinct()
...
Lamar Jackson led Elliot's Last Good Tight End (LGTE) to a historic scoring week for the franchise. His 47 fantasy points shows Lamar is ready to get back to MVP form and may lead the LGTE back to KFL relevance and a fantasy spot. 
...
All the Trey Lance truthers out there will have to wait yet another season as the QB goes down with a season ending injury and lets the (un)Lucky Ducks down with only 2 fantasy points. Luckily for the Ducks, they were able to overcome and eek out a 2 point win, but they will be desperate for a QB as back up Matt Ryan looks like he may be leading one of the worst teams in the league this year. 
