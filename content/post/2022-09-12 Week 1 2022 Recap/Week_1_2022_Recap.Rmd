...
date: "2022-09-12T13:09:13-06:00"
...
kfl_franchises_2022<-ff_franchises(espn_conn_2022)
...
player_rankings<-starters %>%group_by(franchise_name,pos) %>% mutate(rank = dense_rank(desc(player_score)),pos_rank=paste0(pos,"_",rank))%>%arrange(franchise_name,pos,desc(player_score))
...
actual_result<-ff_schedule(espn_conn_2022)%>%select(week,franchise_id,franchise_score)%>%filter(week==1)
...
bb_team<-as.data.frame(bb_team%>%inner_join(kfl_franchises_2022,by=c("franchise_id"="franchise_id"))%>%select(franchise_name.x,logo,player_n,team_score,franchise_score,pts_below_optimal)%>%select(-franchise_id))

bb_schedule<-ff_schedule(espn_conn_2022)%>%filter(week ==week_num)%>%inner_join(bb_team%>%dplyr::select(franchise_id,team_score),by=c("franchise_id"="franchise_id"))%>%inner_join(bb_team%>%dplyr::select(franchise_id,team_score),by=c("opponent_id"="franchise_id"))%>%rename(fran_bb_score = team_score.x, opp_bb_score = team_score.y)%>%mutate(bb_win = ifelse(fran_bb_score>opp_bb_score,1,0))

bb_team<-bb_team%>%inner_join(bb_schedule%>%dplyr::select(franchise_id,bb_win),by=c("franchise_id"="franchise_id"))%>%mutate(bb_loss=week_num-bb_win)
...
bb_table <- bb_team%>%gt()%>%
  text_transform(
   ...
   cols_label(
      ...
      logo = "",
      player_n = "starter count",
      team_score = "Optimal Best Ball Lineup",
      franchise_score = "Actual Score",
      pts_below_optimal = "Points under Optimal",
      bb_win = "Best Ball Win",
      bb_loss = "Best Ball Loss"
    )
