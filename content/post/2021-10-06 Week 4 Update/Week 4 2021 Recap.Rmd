...
date: 2020-10-06T13:09:13-06:00
...
kfl_week_4<-kfl_schedule_2021%>%filter(week==1)
...
Week 4 had a few notable headlines for the KFL. Jeff and Kurt in a battle of brothers tied 97-97 with Jeff's Slugs winning the tie-breaker. It was a tough break for the Tenderloin's who had a 73% win probability. The second tie was in the battle of the beaten. The Sunday Winners (1-3) had an incredibly unlucky stretch to go 0-3 despite 3 consecutive weeks of 100+ points. However, they caught a bit of luck against Nick's Fightin' Ducks (0-4) to win a 80-80 tie-breaker. The Fightin' Ducks are now in sole possession of last place and will look to rebuild for next year in what is likely a lost season.

Both the undefeated Onionhead and Kraken lost in deserving fashion. The Onionheads put up a measly 47 points and let the SWAB get a lucky win where they only had a 12% win probability. The Kraken put up 67 to loss to Scott's Titans 93-67.

Finally, Max's Supermarket Studs and Micheal's GMOs put up big weeks of 110 and 123 respectively, to even their records to 2-2. 
...
kfl_2021_win_prob_data<-kfl_schedule_2021%>%rename( score_for = franchise_score)%>%filter(week<5)
...
wk4_matchups<-kfl_2021_weekly_data%>%filter(week==4 &win==1)
...
wk4_matchups%>%mutate(win_prob=round(win_prob,2),opp_win_prob=round(opp_win_prob,2))%>%gt()%>%gt_img_rows(logo)%>%gt_img_rows(opponent_logo)%>%
  tab_header(title ="Week 4 matchups")%>%
...
## Expected Wins power rankings through week 4

Through Week 4, The Sunday Winners lead our expected wins power ranking. They will need to be hot the rest of the way out though if they want to make the playoffs. The Kraken, SuperMarket Studs, and Onionheads round out the top 4, suggesting the Kraken and Onionheads are the real deal with 3-1 records and expected wins to back it up.

Jeff's Slugs and Cincinnati SWAB are riding some luck to their 3-1 record. Both teams just have 2 expected wins. 

Meanwhile, Nick's Fighting Ducks will see if they can buy a win next week, but they are certainly better than their 0.4 record suggests with 2 expected wins on the season.

Elliott's Warriors find themselves in a familiar place at the basement of KFL with just 1 expected win on the season. 



```{r expected_wins, echo=FALSE,warning=FALSE,fig.width = 8,fig.height=6,message=FALSE}
kfl_wins_over_expected<-kfl_2021_weekly_data%>%select(win_prob,win,franchise_id,franchise_name,logo)%>%group_by(franchise_id,franchise_name,logo)%>%summarise(expected_wins = sum(win_prob),wins = sum(win))%>%mutate(wins_over_expected = expected_wins - wins)%>%arrange(expected_wins)

week_4_table<-kfl_wins_over_expected%>%select(franchise_id,franchise_name,logo,wins,expected_wins,wins_over_expected)%>%mutate(expected_wins = round(expected_wins,1), wins_over_expected = round(wins_over_expected,1))%>%arrange(desc(expected_wins))

week_4_table<-as.data.frame(week_4_table[,c(2:6)])

asp_ratio <- 1.618 

week4_exp_wins<-ggplot(kfl_wins_over_expected,aes(x=reorder(franchise_name,expected_wins),y=expected_wins))+geom_bar(stat="identity")+geom_image(aes(x=franchise_name,y=expected_wins,image=logo), size = 0.05, by = "width", asp = asp_ratio
...
week_4_table%>%gt()%>%gt_img_rows(logo) %>%tab_header(title ="Expected Wins through Week 4")%>%
...
Below are the standings through week 4. The SWAB are on a three game winning streak and Nick's Fightin' Ducks own a 4 game losing streak.

```{r divison records, echo=FALSE,warning=FALSE,fig.width = 10,fig.height=8,message=FALSE}

kfl_standings_wk4<-kfl_2021_weekly_data%>%filter(week<5)
...
standings_df<-joined_df%>%select(franchise_name,logo,division_name,Wins,Losses,outcomes)%>%gt(groupname_col="division_name")%>%gt_plt_winloss(outcomes,max_wins = 14)%>%gt_img_rows(logo)%>%tab_header(title ="Standings Through Week 4")%>%cols_label(franchise_name="Team",logo="",outcomes="Win-Loss Streak")
